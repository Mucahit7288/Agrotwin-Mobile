...
class AppState extends ChangeNotifier {
  ...
  Future<String?> login(String email, String password) async {
    ...
    if (emailNorm.isEmpty || password.isEmpty) return 'E-posta ve şifre gerekli.';
  ...
  Future<String?> register({
    ...
    if (emailNorm.isEmpty || !emailNorm.contains('@')) return 'Geçerli bir e-posta girin.';
    if (password.length < 4)                         return 'Şifre en az 4 karakter olmalı.';
    final usersJson = _prefs.getStringList('auth_users') ?? [];
    for (final row in usersJson) {
      final m = jsonDecode(row) as Map<String, dynamic>;
      if (m['email']?.toString().toLowerCase() == emailNorm) return 'Bu e-posta zaten kayıtlı.';
    }
    usersJson.add(jsonEncode({
  ...
  // ── Sensör & MQTT ─────────────────────────────────────────────────────────────
  void updateSensor(SensorData data) { sensorData = data; notifyListeners(); }
  ...
  // ── Aktüatör Toggleları ───────────────────────────────────────────────────────
  void togglePompa()   { pompaOn   = !pompaOn;   notifyListeners(); }
  void toggleFan()     { fanOn     = !fanOn;     notifyListeners(); }
  void toggleIsitici() { isiticiOn = !isiticiOn; notifyListeners(); }
  void toggleLed()     { ledOn     = !ledOn;     notifyListeners(); }
  void setLedPwm(double v)    { ledPwm      = v; notifyListeners(); }
  void setKontrolModu(int m)  { kontrolModu = m; notifyListeners(); }
  void setRecete(String r)    { seciliRecete = r; notifyListeners(); }

  // ── Simülatör ─────────────────────────────────────────────────────────────────
  void setSimPh(double v)       { simPh       = v; notifyListeners(); }
  void setSimEc(double v)       { simEc       = v; notifyListeners(); }
  void setSimIsikSaat(double v) { simIsikSaat = v; notifyListeners(); }
  void setSimSicaklik(double v) { simSicaklik = v; notifyListeners(); }
  ...
  double get tahminiMaliyet => 80 + simIsikSaat * 2.5 + (simSicaklik > 24 ? 30 : 0);
}
